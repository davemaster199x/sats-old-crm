						...
						<tr class="body_tr jalign_left" <?php echo $row_color; ?>>
							...
							<td>
								...
								if( $row['ts_completed'] == 1 ){ ?>
									<div <?php echo ( isDHAagenciesV2($row['franchise_groups_id'])==true )?'style="display:none;"':''; ?>>
										...
										<img src="images/email_green.png" data-job_id="<?php echo $row['jid']; ?>" data-pdf_type="invoice_cert" class="btn_email_agency <?php echo ( date('Y-m-d',strtotime($row['mm_need_proc_inv_emailed'])) == date('Y-m-d') )?'fadeIt':''; ?>" />
									</div>
								<?php
								}								
								?>							
							...
							<td class="leftGreyBorder">
								<div <?php echo ( isDHAagenciesV2($row['franchise_groups_id'])==true )?'style="display:none;"':''; ?>>
									...
									<img src="images/email_green.png" data-job_id="<?php echo $row['jid']; ?>" data-pdf_type="invoice" class="btn_email_agency <?php echo ( date('Y-m-d',strtotime($row['mm_need_proc_inv_emailed'])) == date('Y-m-d') )?'fadeIt':''; ?>" />
								</div>
							</td>
	...
	jQuery(".btn_email_agency").click(function(){
		...
		if(confirm('Are you sure you sure you want to continue?')){
			...
			jQuery.ajax({
				...
				window.location="/dha_pre_com_jobs.php"
