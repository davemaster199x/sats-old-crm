...
.w3-modal{z-index:3;display:none;padding-top:0px;position:fixed;left:0;top:0;width:100%;height:100%;overflow:auto;background-color:rgb(0,0,0);background-color:rgba(0,0,0,0.4);margin-bottom:100px;}
...
<div id="tabsss" class="c-tabs no-js">
	<div class="c-tabs-nav">
		...
		<a href="#" data-tab_index="13" data-tab_name="propertyme_tenant" class="nav_tenants propertyme_tenant_tab <?php echo ( $tas['tas_connected'] == 0 )?'fadeIt':''; ?>">
			PropertyMe			
		</a>	
	</div>
	<div class="c-tab-tenants is-active" data-tab_cont_name="active_tenant">
		...
		<table id="inner_new_tenants_tbl" border="0" cellpadding="5" cellspacing="4" style="border-bottom:1px solid #FFF !important; border-left:2px solid #FFF !important; margin-left:-3px !important; border-right:2px solid #FFF !important; margin-right:-3px !important;">
			...
			<tr style="height:36px !important;">
				<td>
					<input type="text" name="tenant_firstname" id="tenant_firstname_det<?=$rsTenants['property_tenant_id']?>" class="addinput tenant_fields<?=$rsTenants['property_tenant_id']?>" style="display:none; width:100px !important;" value="<?=$rsTenants['tenant_firstname']?>">
				...
				<td>
					<input type="text" name="tenant_lastname" id="tenant_lastname_det<?=$rsTenants['property_tenant_id']?>" class="addinput tenant_fields<?=$rsTenants['property_tenant_id']?>" style="display:none; width:100px !important;" value="<?=$rsTenants['tenant_lastname']?>">
				...
				<td>
					...
					<a href="tel:0<?=trim($rsTenants['tenant_mobile'])?>">
					<span style="border-bottom:1px solid #b4151b;" class="tenant_labels<?=$rsTenants['property_tenant_id']?> tenant_mobile_field_v2" id="tenant_mobile_lbl<?=$rsTenants['property_tenant_id']?>"><?=$rsTenants['tenant_mobile']?></span>
					</a>
					<?php }?>
				</td>
				<td>
					...
					<span style="border-bottom:1px solid #b4151b;" class="tenant_labels<?=$rsTenants['property_tenant_id']?> tenant_phone_field_v2" id="tenant_landline_lbl<?=$rsTenants['property_tenant_id']?>"><?=$rsTenants['tenant_landline']?></span>
					</a>
					<?php }?>
				</td>
				<td>
					<input type="email" name="tenant_email" id="tenant_email_det<?=$rsTenants['property_tenant_id']?>" class="addinput tenant_fields<?=$rsTenants['property_tenant_id']?>" style="display:none; width:200px !important;" value="<?=$rsTenants['tenant_email']?>">
				...
				<td>
					...
					<a target="_blank" id="custom_email_link" href="send_email_template.php?job_id=<?=$job_id?>&to_email=<?=$rsTenants['tenant_email']?>">
						<img src="/images/button_icons/mail-tenant.png" class="email_icon default-buttton<?=$rsTenants['property_tenant_id']?>" style="cursor: pointer;" />
					</a>
					<?php }else{ ?>
					<img src="/images/button_icons/mail-tenant-disable.png" class="email_icon default-buttton<?=$rsTenants['property_tenant_id']?>"  />									
					<?php }?>

					<?php 
					if( $propertyme_api->checkSmsforToday($job_id) ){ ?>
					<div class="row" style="display:inline-block; default-buttton<?=$rsTenants['property_tenant_id']?>">
						<?php if($rsTenants['tenant_mobile'] != ""){?>
						<img src="/images/button_icons/sms-tenant.png" onclick="document.getElementById('<?=$rsTenants['property_tenant_id']?>').style.display='block'" class="sms_icon default-buttton<?=$rsTenants['property_tenant_id']?>" style="cursor: pointer;" />									
						<?php }else{ ?>
						<img src="/images/button_icons/sms-tenant-disable.png" class="sms_icon default-buttton<?=$rsTenants['property_tenant_id']?>"  />									
						<?php }?>
					</div> 
					<?php }?>

					<div id="<?=$rsTenants['property_tenant_id']?>" class="w3-modal">
					    <div class="w3-modal-content">
					      <div class="w3-container" style="padding-bottom:100px !important;">
					        <span onclick="document.getElementById('<?=$rsTenants['property_tenant_id']?>').style.display='none'" class="w3-button w3-display-topright">&times;</span>
					        
					        <div class="sms_div sms_div_<?=$rsTenants['property_tenant_id']?>">
					        	<h2 class="heading">SMS Tempalte</h2>
								<?php
	...
	<div class="c-tab-tenants inactive_tenant_tab_div"  data-tab_cont_name="inactive_tenant">
		...
		<table border="0" cellpadding="5" cellspacing="4" style="border-bottom:1px solid #FFF !important; border-left:2px solid #FFF !important; margin-left:-3px !important; border-right:2px solid #FFF !important; margin-right:-3px !important;">
			...
			<tr style="height:36px !important;">
				...
				<td class="jcenter">
					<center>
						<a class="reactivate-tenant" tid="<?=$rsInActiveTenants['id']?>">
