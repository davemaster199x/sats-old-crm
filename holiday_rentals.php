...
$custom_query = " AND p.`holiday_rental` = 1 AND j.status NOT IN('Completed','Cancelled','Merged Certificates','Booked','Pre Completion') ";
			...
			<tr class="toprow jalign_left">
				...
				?>
						<tr class="body_tr jalign_left" <?php echo $row_color; ?>>
							...
							<td><input type="text" class="jcomment" value="<?php echo $row['comments']; ?>" /></td>
		...
		<div style="margin-top: 15px; float: right; display:none;" id="map_div">
			...
			<select id="maps_tech">
				...
				while($tech = mysql_fetch_array($tech_sql)){ ?>
					...
					<option value="<?php echo $tech['id']; ?>"><?php echo "{$tech['first_name']} {$tech['last_name']}"; ?>
						<?php 
...
jQuery(document).ready(function(){
	...
	jQuery(".jcomment").change(function(){
		
		var job_id = jQuery(this).parents("tr:first").find('.hid_job_id').val();
		var jcomment = jQuery(this).val();		
		
		jQuery("#load-screen").show();
		jQuery.ajax({
			type: "POST",
			url: "ajax_update_job_comments.php",
			data: { 
				job_id: job_id,
				jcomment: jcomment
			}
