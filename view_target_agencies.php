...
if(isset($_GET['del_id']) && is_numeric($_GET['del_id']))
{
	...
	$query = "DELETE FROM agency WHERE status = 'target' AND agency_id = '" . $_GET['del_id'] . "' LIMIT 1";
...
function get_target_agency_list($start="",$limit="",$state="",$region="",$sales_rep="",$phrase="",$sort="",$order_by="",$agency_using_id){
	...
	*/
	...
	$sql = "
			SELECT *, a.agency_name, a.address_1, a.address_2, a.address_3, a.state, a.postcode, a.status, a.agency_id, a.tot_properties
			FROM agency AS a
			LEFT JOIN `postcode_regions` AS pr ON a.`postcode_region_id` = pr.`postcode_region_id`
			WHERE  a.status='target'		
			AND a.`country_id` = {$_SESSION['country_default']} 
			".$appendquery." 
			GROUP BY a.agency_id {$sort_str} {$limit_str}
	";
...
if( $search_flag == 1 ){
	...
	$result = get_target_agency_list($offset,$limit,$state,$region,$sales_rep,$phrase,$sort,$order_by,$agency_using);
	$ptotal = mysql_num_rows(get_target_agency_list('','',$state,$region,$sales_rep,$phrase,$sort,$order_by,$agency_using));
  ...
  <div class="sats-middle-cont">
    ...
    <div class="sats-breadcrumb">
      <ul>
        ...
        <li class="other first"><a title="View Target Agencies" href="<?php echo $_SERVER['PHP_SELF']; ?>"><strong>View Target Agencies</strong></a></li>
      </ul>
    </div>
...
<table cellspacing="0" cellpadding="0">
        <tbody><tr class="tbl-view-prop">
          <td>
          <form action='<?=URL;?>view_target_agencies.php' method='post' name='search' id='search' class="vw-trg-agnc" style="margin:0px;">
			...
			if(ifCountryHasState($_SESSION['country_default'])==true){ ?>
				<div class="fl-left">
					...
					<select name="searchstate">
							<option <?php echo $state == ''? 'selected="selected"': '';?> value=''>--- Select ---</option>
			...
			?>
			  ...
			  <?php
			  $sr_sql = getAgencySalesRep('target');			  
			  ?>
			  <div class="fl-left">
				...
				<select name="sales_rep">
					...
					while($sr = mysql_fetch_array($sr_sql)){ ?>
						<option value="<?php echo $sr['salesrep'] ?>" <?php echo ($sr['salesrep']==$sales_rep)?'selected="selected"':''; ?>><?php echo $sr['FirstName'] ?> <?php echo $sr['LastName'] ?></option>
					<?php
					}
					?>
				</select>
			  ...
			  <div class="fl-left">
				...
				<select name="agency_using">
					...
					while($au = mysql_fetch_array($au_sql)){ ?>
						<option value="<?php echo $au['agency_using_id']; ?>" <?php echo ($au['agency_using_id']==$agency_using)?'selected="selected"':''; ?>><?php echo $au['name']; ?></option>
					<?php
					}
					?>
					</select>		
...
<table width="100%" cellspacing="1" cellpadding="5" border="0" class="table-left tbl-fr-red">	
	...
	<?php
		...
		if( $search_flag == 1 && mysql_num_rows($result)>0 ){
	   ...
	   $odd++;
		if (is_odd($odd)) {
		 ...
		 // print the values of the attributes
			...
			echo "<td>";		
			echo "<a href='{$ci_link}' target='_blank'>{$row['agency_name']}</a>";
			echo "</td>\n";
