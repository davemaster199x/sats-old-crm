				...
				if(mysql_num_rows($menu_sql)>0){ 
					...
					<tbody class="crm_menu_tbody">
						<tr class="toprow jalign_left jmain_row crm_menu_row">
							...
							$sc_sql = $crm->getAllStaffClasses();
							while( $sc = mysql_fetch_array($sc_sql) ){ ?>
								<td><input type="checkbox" title="<?php echo $sc['ClassName'] ?>" class="staff_class_chk menu_staff_class_chk" <?php echo ( isMenuAllowed_StaffClass($menu['menu_id'],$sc['ClassID']) == true )?'checked="checked"':''; ?> value="<?php echo $sc['ClassID']; ?>" /></td>
							<?php
							}
							?>
							<td>
						...
						<tr class="toprow jalign_left crm_pages_row <?php echo ( is_numeric($page['active']) && $page['active']==0 )?'fadeOutText':''; ?>">
							...
							$sc_sql = $crm->getAllStaffClasses();
							while( $sc = mysql_fetch_array($sc_sql) ){ ?>
								<td><input type="checkbox" title="<?php echo $sc['ClassName'] ?>" class="staff_class_chk page_staff_class_chk" <?php echo ( isPageAllowed_StaffClass($page['crm_page_id'],$sc['ClassID']) == true )?'checked="checked"':''; ?> value="<?php echo $sc['ClassID']; ?>" /></td>
							<?php
							}
							?>
