...
$mr_id = mysql_real_escape_string($_GET['mr_id']);



$map_routes_sql = mysql_query("
	SELECT * 
	FROM  `map_routes` 
	WHERE  `map_routes_id` = {$mr_id}
");
$map_routes = mysql_fetch_array($map_routes_sql);
$tech_id = $map_routes['tech_id'];
$day = date("d",strtotime($map_routes['date']));
$month = date("m",strtotime($map_routes['date']));
$year = date("Y",strtotime($map_routes['date']));
$date = $map_routes['date'];
$sub_regions = $map_routes['sub_regions'];
...
$title = (mysql_num_rows($t_sql)>0)?"Map - {$t['FirstName']} {$t['LastName']}":"Tech Booking";
...
// refresh list to newest
if(mysql_num_rows($map_routes_sql)>0){

	// updates the map listing to it's newest state
	updateMapListing2($tech_id,$date,$sub_regions,$_SESSION['country_default']);
  ...
  <div class="sats-middle-cont">
    ...
    <div class="sats-breadcrumb">
      <ul>
        ...
        <li class="other first"><a title="<?php echo $title; ?>" href="/tech_booking.php?mr_id=<?php echo $mr_id; ?>"><strong>Maps</strong></a></li>
      </ul>
    </div>
...
<?php
	...
	$sa_sql = mysql_query("
		...
		WHERE sa.`staff_accounts` ={$tech_id}
	");
...
$jr_list2 = getJobsByRegionSort($tech_id,$date,$sub_regions,$_SESSION['country_default']);

//$jr_count = mysql_num_rows($jr_list);
$jr_count = mysql_num_rows($jr_list2);

$comp_count = 0;
$jr_arr = array();
while($jr = mysql_fetch_array($jr_list2)){
...
.fl-left input.addinput {
  ...
  <div class='fl-left' style='float: left;'>
	...
	<input type="hidden" id="orig_btn_txt" value="<?php echo ( $map_routes['start']!="" && $map_routes['end']!="" )?'EDIT':'SET'; ?> Start & End"  />
<button type="button"  id="btn_set_start_end" jclicked="0" class="submitbtnImg blue-btn"><?php echo ( $map_routes['start']!="" && $map_routes['end']!="" )?'EDIT':'SET'; ?> Start & End</button>
</div>
...
<div id="start_end_main_div" style="float: left; width: auto; margin-bottom: 2px; display:none;">
	...
	<select id="start_point" style="float:none; width:100px;">
		...
		while($acco = mysql_fetch_array($acco_sql)){ ?>
			<option value="<?php echo $acco['accomodation_id']; ?>" <?php echo ($acco['accomodation_id']==$map_routes['start'])?'selected="selected"':''; ?>><?php echo $acco['name']; ?></option>
		<?php	
		}
		?>
	</select>
	...
	<select id="end_point" style="float:none; width:100px;">
			...
			");
			while($acco = mysql_fetch_array($acco_sql)){ ?>
				<option value="<?php echo $acco['accomodation_id']; ?>" <?php echo ($acco['accomodation_id']==$map_routes['end'])?'selected="selected"':''; ?>><?php echo $acco['name']; ?></option>
			<?php	
			}
			?>
		</select>
...
<tr>
	...
	<td>
		<select name="keys_agency" id="keys_agency" style="width:200px;">
			...
			$region_str = getRegionFilterforQuery($tech_id,$date,$sub_regions,$_SESSION['country_default']);
...
<div class="vosch-tp aviw_drop-h" style="background-color:#e0fde0;">

<div style="float: none;" class="fl-left">
	<div style="float: left; margin-right: 10px;">
	Date: <input type="text" readonly="readonly" name="hdate" class="addinput" style="width: 75px;" value="<?php echo "{$day}/{$month}/{$year}"; ?>" />
	</div>
	
	
	<div style="float: left; margin-right: 10px;">
	Tech: <input type="text" readonly="readonly" name="htech" class="addinput" style="width: auto;" value="<?php echo "{$sa['FirstName']} {$sa['LastName']}"; ?>" />
	</div>

	<div style='float: left; margin-right: 10px;'>Target: <input type='text' style='width: 35px;' class='addinput' value='<?php echo $tot_tar['booking_target']; ?>' readonly='readonly' /></div>
    <div style='float: left; margin-right: 10px;'>Booked:  <input type='text' style='width: 30px;' class='addinput' value='<?php echo $tot_job['jcount']; ?>' readonly='readonly' /></div>
	 <div style='float: left; margin-right: 10px;'>DKs:  <input type='text' style='width: 30px;' class='addinput' value='<?php echo $tot_dk['jcount']; ?>' readonly='readonly' /></div>
	 <div style='float: left; margin-right: 10px;'>Billables:  <input type='text' style='width: 30px;' class='addinput' value='<?php echo $tot_bill['jcount']; ?>' readonly='readonly' /></div>
	<div style='float: left; margin-right: 10px;'>Completed:  <input type='text' style='width: 50px;' class='addinput' value='<?php echo "{$comp_count}/{$jr_count}"; ?>' readonly='readonly' /></div>
...
<tr class="nodrop nodrag" style="background-color:<?php echo $start_row_color; ?>">
<td colspan="3">
<?php 
// start
echo $ctr; 
...
for($j=2;$j<=$total_map_routes;$j++){ 
	...
	?>
		<tr id="key_routes_id:<?php echo $kr_arr[$j]['key_routes_id']; ?>" style="background-color:<?php echo ($kr_arr[$j]['completed']==1)?'#c2ffa7':'#eeeeee'; ?>;">
			...
			<td>&nbsp;</td>
			<td>
			<?php $ci_link = $crm->crm_ci_redirect("/agency/view_agency_details/{$kr_arr[$j]['agency_id']}"); ?>
			<a href="<?php echo $ci_link; ?>"><?php echo "{$kr_arr[$j]['address_1']} {$kr_arr[$j]['address_2']}, {$kr_arr[$j]['address_3']}"; ?></a></td>			
	...
	?>
		<tr id="jobs_id:<?php echo $jr_arr[$j]['jid']; ?>" style="background-color:<?php echo $bgcolor; ?>">
			...
			<a href="<?php echo $ci_link; ?>" class="agency_td"><?php echo $jr_arr[$j]['agency_name']; ?></a></td>
			<td><?php echo $ci_link2; ?></td>
			<td>
			<?php
			...
			<td>
				...
				<input type="hidden" class="is_dk_allowed" value="<?php echo $jr_arr[$j]['allow_dk']; ?>" />
				<input type="hidden" class="agency_id" value="<?php echo $jr_arr[$j]['agency_id']; ?>" />
			</td>
			<td><?php echo $j; ?></td>
		</tr>
	<?php	
...
<tr class="nodrop nodrag" style="background-color:<?php echo $end_row_color; ?>;">
<td colspan="3">
<?php 
if($mp['start']=="" && $mp['end']==""){
...
jQuery(document).ready(function(){
	...
	jQuery("#btn_assign_dk").click(function(){
		...
		jQuery(".del_map_route:checked").each(function(){
			...
			var chunk = chk_val.split(":");
			console.log(chunk);
			var job_type = chunk[0];
			var jid = chunk[1];
			if(job_type=="job_id"){
			...
			if(agency_arr.length>0){
				...
				if(confirm(msg)){
					...
					jQuery.ajax({
						...
						//location.reload();
						window.location='/tech_booking.php?mr_id=<?php echo $mr_id; ?>';
					});	
