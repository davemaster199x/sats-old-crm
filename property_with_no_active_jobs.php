...
echo "<h1>No Active Jobs Properties</h1>";
...
function hasActiveJobs($property_id){
	...
	WHERE `j`.`del_job` = 0
	AND (
        `j`.`job_type` = 'Yearly Maintenance' OR
        `j`.`job_type` = 'Once-off'     
    )
    AND (
        `j`.`status` = 'Completed' OR
        `j`.`status` = 'To Be Booked' OR
        `j`.`status` = 'Booked' OR
        `j`.`status` = 'Merged Certificates' OR
        `j`.`status` = 'Pre Completion'
    )
...
<table>
	...
	while( $row = mysql_fetch_array($sql) ){ 
		...
		if( hasActiveJobs($row['property_id']) == 0 ){
		?>
			<tr>
                <td><?php echo $i; ?></td>
				<td>
...
<p><b>Number of Properties with No Active Jobs: <?php echo $i; ?></b></p>
