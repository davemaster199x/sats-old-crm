<?
    ...
    //$pagecount = $pdf->setSourceFile(getcwd() .'/inc/templates/rta-entry-notice-form9_10-01-2018.pdf');
    $pagecount = $pdf->setSourceFile(getcwd() .'/inc/templates/en_qld_v2.pdf');
    ...
    // Address L1
    $pdf->Cell(21,0,"");     
    //$pdf->Cell(0,0, $job_details['address_1'] . " " . $job_details['address_2']);
    $pdf->Cell(0,0, $incov_val1);

    // Suburb and Postcode
    $pdf->Ln(6.6);
    $pdf->Cell(21,0,"");     
    //$pdf->Cell(70,0, $job_details['address_3'] . " " . $job_details['state']);
    ...
    $pdf->Cell(137,0, $job_details['FirstName'] . " " . substr($job_details['LastName'], 0, 1).'.');
    //$pdf->Cell(137,0, $job_details['FirstName'] . " " . $job_details['LastName']);
    $pdf->Cell(30,0, $tenant_number);
    ...
    $pdf->Cell(45,0, ( $job_details['en_date_issued'] != '' )?date('l',strtotime($job_details['en_date_issued'])):null ); //eg Sunday
    ...
    $pdf->Cell(23,0, ( $job_details['en_date_issued'] != '' )?date('Y',strtotime($job_details['en_date_issued'])):null ); // yy
    ...
    $pdf->Cell(46,0, $job_details['booking_date_name']); // Smoke alarm act
    $pdf->Cell(9,0, $job_details['booking_date_day']); // dd
    $pdf->Cell(9.2,0, $job_details['booking_date_month']); // mm
    $pdf->Cell(21,0, $job_details['booking_date_year']); // yy
    ...
    //$pdf->Image(__DIR__ . "/../images/signature-entrynotice.png", 100, 258, 50); // Manually position image on PDF
    $pdf->Image(__DIR__ . "/../images/DK_signature.png", 102, 253, 63); // Manually position image on PDF
    ...
    $pdf->addPage(); 
    $pdf->setSourceFile(getcwd() .'/inc/templates/en_qld_v2_p2.pdf');
    $tplidx = $pdf->importPage(1, '/MediaBox');       
    $pdf->useTemplate($tplidx, 0, 0, 210); 
