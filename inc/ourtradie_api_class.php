...
class OurtradieApi
{
	...
	{	
		...
		if (array_key_exists('expires_in', $token) && $token['created'] + $token['expires_in'] < time() + 20)
		{
			...
			$token = $this->refreshToken($token['refresh_token']);5fd756ac806880edd20c180c10b1127df03379b2
		}
		*/
		$headers = array('Content-Type:multipart/form-data', 'Authorization: Bearer '.$files['access_token']);
		
		//$headers = array('Content-Type:multipart/form-data', 'Authorization: Bearer 5fd756ac806880edd20c180c10b1127df03379b2');
		/*
		echo $apiMethod;
		echo "<br />";
		print_r($data);
	...
	public function refreshToken($token = null)
	{
		if (!$this->getOption('userefresh'))
		{
			echo 'Refresh token is not supported for this OAuth instance.';
			exit;
		}
		...
		*/
		...
		if (!empty($response['access_token']))
		{
			...
			print_r($strings);
			echo "<br />";
			echo $this->options['redirect_uri'];
			echo "<br />";
